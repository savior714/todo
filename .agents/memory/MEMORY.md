...
- 2026-06-16: DB UNIQUE 제약 마이그레이션 + Admin 프로필 삭제 UI 연결 — `db/migrations/0006_add_unique_constraints.sql` 생성 및 Turso 적용 완료 (`homework_types` family+child_group+title, `quick_actions` family+label UNIQUE 인덱스). `app/admin/profile-delete-section.tsx` 클라이언트 컴포넌트 생성, `/admin` 페이지에 연결 — 본인 프로필만 삭제 가능(confirm + 서버 측 검증), 삭제 시 events/daily_pins/homework_logs/routine_logs 함께 정리. `0000_good_prism.sql` 마이그레이션 충돌 유발 파일 제거, drizzle-kit stale meta 정리. `bun run build`·`typecheck:strict`·`db:migrate`·`plan-lint` 모두 통과.
...
- 2026-05-12: 타임라인 카드 클릭 시 `TimelineEventDetailModal` 상세·실행 취소(가능할 때만). 미완료 숙제는 열 날짜(로컬·오늘 이하)에 점선 카드로 노출 후 모달에서 `completeHomework(id, dateKey)` 완료 체크. `completeHomework` 두 번째 인자(선택)와 `lib/homework-date-key.ts` 검증, `TimelineFeedSection`이 `homework_types`·`homework_logs` 범위 조회 후 전달. `bun run lint`·`typecheck:strict`·`test` 통과.
...
- 2026-05-11: 등원(`school_dropoff`)·하원(`school_pickup`) 기록 시 모달에서 **주원이/승원이**(`kid7`/`kid4`) 선택·**장소(선택)** 입력 추가. `metadata.schoolRun` 검증·표시(`summarizeEventMetadataForDisplay`)·`events.target === child`로 `lib/event-metadata.ts`·`RecordEventModal`·`TimelineFeed`(대상 한글 표기)·`CRITICAL_LOGIC.md` 반영. `bun run lint`·`typecheck:strict`·`test`·`verify_korean_text` 통과.
...
- 2026-05-10: 실행 취소를 액션별로 분리 — `lib/event-undo-policy.ts` SSOT(저위험 24h, 투약 30m). `app/actions/events.ts` `undoEvent`·`TimelineFeed` 버튼 노출이 동일 함수 사용. `CRITICAL_LOGIC.md`·`PRD.md`·`TRD.md` 정합, 계약 테스트 1건 추가. `bun test`·`bun run lint`·`bun run typecheck:strict` 통과.
...
- 2026-05-10: Git 정리: 타임라인·스펙·ai-log 워크플로·E2E 계약 커밋. `tools/ai_worklog` 로컬 심볼릭 링크는 `.gitignore`로 제외(절대 경로 공유 방지). `just ci`, `bun run lint`·`typecheck:strict`·`test` 통과 확인.
...
- 2026-06-16: Vercel preview OAuth 로그인 수정 — `instrumentation.ts`에서 preview 환경 `AUTH_URL` 삭제 대신 `VERCEL_URL` 기반 동적 설정(`https://${VERCEL_URL}`)으로 변경. 기존 `delete` 방식은 NextAuth module init 시 `setEnvDefaults()`가 이미 `AUTH_URL` 읽어서 무력화됨. `bun run lint`·`typecheck:strict`·`test`(33 pass) 통과.
- 2026-06-16: 대시보드 React #418 하이드레이션 에러 근본 수정 — `TimelineFeed.tsx` line 416의 `toLocaleTimeString(undefined, ...)`를 `"ko-KR"` 로케일 + `hour12: false`로 명시하여 SSR/CSR 시간 포맷 불일치 해결. `bun run lint`·`typecheck:strict`·`test`(33 pass)·`just ci` 통과.
- 2026-06-16: 대시보드 핵심 기능 복구 — `lib/auth/session.ts`에서 React `cache()` 제거하여 Server Action 컨텍스트에서 `getActiveProfileContext()`가 정상 동작하도록 수정(퀵 액션 기록 500 에러 근본 해결). `RecordEventModal.tsx`에서 medication 대상 라벨을 `SCHOOL_CHILD_LABEL` 패턴으로 통일 + actionType 전환 시 `genericNote` 명시적 초기화. `bun run lint`·`typecheck:strict`·`just plan-lint` 통과.
...
- 2026-05-09: `just ci` 실행 시 `lint-fix`, `memory-verify` 순으로 통과하고 `ci: minimal checks passed`를 확인함.
...
## Consistency Issues
- 2026-05-09: `AGENTS.md`에서 필수로 참조하는 `.agents/memory/ADAPTIVE_GUIDELINES.json` 파일이 현재 저장소에 없음. (영향: Guideline Compliance 항목 실적용 불가)
  - 권장 조치: `.agents/memory/ADAPTIVE_GUIDELINES.json` 초기 템플릿 추가 및 태스크별 가이드라인 매핑 체계 도입.
...
- 2026-05-09: `.agents/memory/ADAPTIVE_GUIDELINES.json` 파일 복구됨. 현재는 스키마 유지 + `guidelines: []` 초기 상태로 리셋됨.
