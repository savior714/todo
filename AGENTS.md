...
최종적으로 사용자에게 보고 할 때, **한국어**로 최대한 답변할 수 있는 것들은 하도록 한다.
...
## 3.9 Edge Case Consideration

구현 시 다음 경계 조건을 반드시 고려한다.

* **Null / Undefined / Nullish**: `null`, `undefined`, `""`, `0`, `false` 중 실제 비어있는 값과 의도된 falsy 값을 구분
* **빈 배열 / 빈 객체**: `[]`, `{}` 처리 — 순회 시 동작, 렌더링 시 상태, API 응답 분기
* **단일 항목**: 배열 길이가 1일 때의 경로 분기 (예: 정렬, 슬라이싱, UI 레이아웃)
* **대용량 데이터**: 페이지네이션, 메모리 사용량, 렌더링 성능 — 즉시 로딩 vs 지연 로딩
* **동시성 / Race Condition**: 빠른 연속 클릭, 중복 요청, optimistic update 롤백
* **시간대 / 로케일**: `Date` 객체 처리 시 UTC vs 현지 시간, DST 전환일
* **숫자 정밀도**: 부동소수점 연산 (예: `0.1 + 0.2`), 큰 정수 (`Number.MAX_SAFE_INTEGER` 초과)
* **입력 검증**: 극단적 길이, 특수문자, 인코딩 문제, SQL/JS injection 시도
* **네트워크 장애**: 타임아웃, 재시도, 오프라인 상태, 부분 실패 시 롤백
* **권한 경계**: 역할별 접근 제어 — 권한 없는 사용자의 비정상적 액션 시 Graceful degradation

고려 사항이 불분명할 때는:

```text
구현 → 테스트 작성 (경로별) → 리뷰에서 검토
```

단, 과도한 예외 처리는 3.2(Simplicity First)에 위배되므로, **실제 발생 가능성이 있는** 경계 조건에 집중한다.
